HelloworldController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('helloworld', this.getCriteria(), sort);
    },
...
{age: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Helloworld.find(this.getCriteria(), sort);

        return this.render('helloworldIndex', {
            data: {
    ...
    view: function() {
        return this.render('helloworldView', {
            data: {
...
age: t.find('#age').value,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Helloworld.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', TAPi18n.__("insert_success", "") + ".");
                Router.go('helloworldView', {_id: _id});
            });
        }
        return this.render('helloworldInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Helloworld.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
        ...
        return this.render('helloworldUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Helloworld.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', TAPi18n.__("remove_success", "") + ".");
        });
    },
    _loadModel: function(_id) {
        return Helloworld.findOne(_id);
    },
});